...
from cfme.exceptions import InstanceNotFound, OptionNotAvailable, DestinationNotFound, \
    BlockTypeUnknown, TemplateNotFound
...
class InstanceDetails(CFMENavigateStep):
    ...
    def step(self):
        ...
        try:
            sel.click(list_table.find_row_by_cell_on_all_pages(
                {'Name': self.obj.name,
        ...
        logger.warning('Matching instance only using name: {}'.format(self.obj.name))
